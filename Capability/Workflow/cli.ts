...
import { BUSINESS_ACTIONS } from "../../Pipeline/Business/mcp-server.ts";
import { SCIENCE_ACTIONS } from "../../Pipeline/Science/mcp-server.ts";
import { CONTENT_ACTIONS } from "../../Pipeline/Content/core.ts";
import { DESIGN_ACTIONS } from "../../Pipeline/Design/core.ts";
import { CHROME_ACTIONS } from "../Browser/mcp-server.ts";
import { MESSAGE_ACTIONS } from "../Message/mcp-server.ts";
import { SECRET_ACTIONS } from "../Secret/mcp-server.ts";
import { INFRA_ACTIONS } from "../Infra/core.ts";
import { COMPANY_ACTIONS } from "../../Pipeline/Company/core.ts";
import { AI_ACTIONS } from "../AI/operation.ts";
...
`);
    ...
    case "autopilot": {
      ...
      if (sub === "status") {
        ...
        console.log("Adaptive Respiration: SURGE (2s) | GROWTH (5s) | CRUISE (8s) | REST (10s)\n");
        let cycle = 1;
        let idleStreak = 0;
        while (true) {
