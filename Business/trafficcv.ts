/**
 * TrafficCV (Traffic Analytics & Web Forensics Client)
 *
 * Provides web traffic estimation, visitor channel breakdown (Organic, Direct, Referral),
 * geographic distribution, top page analytics, and competitor traffic benchmarks.
 */
...
export type TrafficCvChannelBreakdown = {
  domain: string;
  channels: {
    direct: number; // percentage (0-100)
...
export class TrafficCvClient {
  ...
  async getDomainOverview(domain: string): Promise<TrafficCvDomainOverview> {
    ...
    // Baseline deterministic traffic telemetry model
